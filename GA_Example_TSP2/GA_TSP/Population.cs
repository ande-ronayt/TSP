...
namespace GA_TSP
{
    public class Population
    {
        ...
        public double maxBalanceFit { get; set; }
        // ctor
        ...
        {
            this.maxBalanceFit = this.p.Max(x => x.balancedFitness);
        ...
        public Population elite(int nn)
        {
            //var n = nn/2;
            var n = 3;
            ...
            for (int i = 0; i < n; ++i)
            {
                best.Add(tmp.findBest(balanced: true));
                tmp = new Population(tmp.p.Except(best).ToList());
            }
        ...
        public Tour findBest(bool balanced = false)
        {
            if (balanced)
            {
                foreach (Tour t in this.p)
                {
                    if (t.balancedFitness == this.maxBalanceFit)
                        return t;
                }
            }
